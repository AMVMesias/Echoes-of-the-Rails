...
func update_pressure(delta: float, throttle: float, brake_input: float) -> void:
	var target_pressure: float = lerpf(idle_pressure, max_pressure, clampf(throttle, 0.0, 1.0))
	if brake_input > 0.05:
		target_pressure = max(idle_pressure * 0.7, target_pressure - brake_input * 25.0)

	var rate: float = pressure_rise_rate if target_pressure > current_pressure else pressure_fall_rate
	...
	var next_warning: bool = current_pressure >= warning_pressure
